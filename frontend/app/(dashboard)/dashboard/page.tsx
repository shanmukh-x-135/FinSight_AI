...
export default function DashboardPage() {
  ...
  const sentimentCounts = data.sentiment.reduce((counts, row) => {
    if (row.latest_sentiment == null) counts.unavailable += 1;
    else if (Math.abs(row.latest_sentiment) < 0.05) counts.neutral += 1;
    else if (row.latest_sentiment > 0) counts.positive += 1;
    else counts.negative += 1;
    return counts;
  }, { positive: 0, neutral: 0, negative: 0, unavailable: 0 });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
        ...
        <Panel><SectionHeader title="News tone" description="Latest daily sentiment across the active universe." action={<Link href="/news" className="text-xs font-medium text-primary hover:underline">Review sources</Link>} />{sentimentTotal === 0 ? <div className="mt-4"><DataState kind="empty" title="No recent sentiment data" description={`No tagged news sentiment is available for ${sentimentCounts.unavailable} active stocks. Missing coverage is not classified as neutral.`} /></div> : <div className="mt-6"><div role="img" aria-label={`${sentimentCounts.positive} positive, ${sentimentCounts.neutral} neutral, ${sentimentCounts.negative} negative sentiment readings; ${sentimentCounts.unavailable} unavailable`} className="flex h-3 overflow-hidden rounded-full bg-muted"><span className="bg-positive" style={{ width: `${sentimentCounts.positive / sentimentTotal * 100}%` }} /><span className="bg-muted-foreground/45" style={{ width: `${sentimentCounts.neutral / sentimentTotal * 100}%` }} /><span className="bg-negative" style={{ width: `${sentimentCounts.negative / sentimentTotal * 100}%` }} /></div><div className="mt-4 grid grid-cols-3 text-center text-xs"><div><p className="text-lg font-semibold text-positive">{sentimentCounts.positive}</p><p className="text-muted-foreground">positive</p></div><div><p className="text-lg font-semibold">{sentimentCounts.neutral}</p><p className="text-muted-foreground">neutral</p></div><div><p className="text-lg font-semibold text-negative">{sentimentCounts.negative}</p><p className="text-muted-foreground">negative</p></div></div>{sentimentCounts.unavailable > 0 && <p className="mt-3 text-center text-xs text-muted-foreground">{sentimentCounts.unavailable} stocks have no recent tagged coverage.</p>}<div className="mt-5 border-t border-border/60 pt-4 text-xs text-muted-foreground"><span className="font-medium text-foreground">Technical regime:</span> {data.technical.above_ema20_count} of {data.technical.stocks_with_indicators} above EMA 20 · {data.technical.positive_macd_count} positive MACD</div></div>}</Panel>
      </div>
