...
export default function HistoryPage() {
  ...
  return <div className="space-y-5">
    ...
    <MetricStrip items={[{ label: "Comparable sessions", value: stats.sample_size, detail: `Top-${stats.k} nearest states` }, { label: "Closed higher", value: bullPct == null ? "—" : `${bullPct}%`, detail: "Following session", tone: bullPct == null ? "neutral" : bullPct >= 50 ? "positive" : "negative" }, { label: "Average next session", value: ratioPct(stats.avg_next_day_return), detail: "Observed, not forecast", tone: stats.avg_next_day_return == null ? "neutral" : stats.avg_next_day_return >= 0 ? "positive" : "negative" }, { label: "Observed range", value: `${ratioPct(stats.worst_case_return)} to ${ratioPct(stats.best_case_return)}`, detail: "Worst to best" }]} />
    <div className="grid gap-4 xl:grid-cols-[1.2fr_.8fr]"><Panel><SectionHeader title="Observed next-session outcomes" description={`${stats.sample_size} nearest sessions in the current result set.`} /><HistoryOutcomeChart result={result} /></Panel><Panel><SectionHeader title={`Current state · ${result.query_date}`} description="Transparent regime inputs and data quality." /><dl className="mt-6 grid grid-cols-2 gap-3">{[["Equal-weight return", ratioPct(summary.avg_return)], ["Advancers", ratioPct(summary.pct_advancers, 1, false)], ["A/D ratio", summary.advance_decline_ratio.toFixed(2)], ["Median RSI", (summary.median_rsi ?? summary.avg_rsi).toFixed(1)], ["Breadth regime", summary.breadth_regime ?? "—"], ["Momentum regime", summary.momentum_regime ?? "—"], ["Volatility regime", summary.volatility_regime ?? "—"], ["Relative volume", summary.median_relative_volume == null ? "—" : `${summary.median_relative_volume.toFixed(2)}×`]].map(([label, value]) => <div key={label} className="surface-subtle p-3"><dt className="text-xs text-muted-foreground">{label}</dt><dd className="financial-number mt-1 font-semibold capitalize">{value}</dd></div>)}</dl>{stats.ci_low != null && stats.ci_high != null && <div className="mt-4 rounded-lg bg-confidence/8 p-3"><p className="text-xs text-muted-foreground">95% confidence interval</p><p className="financial-number mt-1 text-sm font-semibold text-confidence">{ratioPct(stats.ci_low)} to {ratioPct(stats.ci_high)}</p></div>}</Panel></div>
    {selected && <Panel><SectionHeader title={`Why ${selected.date} is similar`} description={`${(selected.similarity_score * 100).toFixed(1)}% normalized regime similarity · grouped agreements, divergences, and observed outcomes.`} action={<label className="text-xs text-muted-foreground">Compare session <select aria-label="Compare historical session" value={selected.date} onChange={(event) => setSelectedDate(event.currentTarget.value)} className="ml-2 rounded-md border border-input bg-background px-2 py-1 text-foreground">{result.similar_sessions.map((session) => <option key={session.date} value={session.date}>{session.date} · {(session.similarity_score * 100).toFixed(0)}%</option>)}</select></label>} /><div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="surface-subtle p-3"><p className="text-xs text-muted-foreground">Next session</p><p className="financial-number mt-1 font-semibold">{ratioPct(selected.next_day_return)}</p></div><div className="surface-subtle p-3"><p className="text-xs text-muted-foreground">Next 5 sessions</p><p className="financial-number mt-1 font-semibold">{ratioPct(selected.forward_5_session_return ?? null)}</p></div><div className="surface-subtle p-3"><p className="text-xs text-muted-foreground">Maximum drawdown</p><p className="financial-number mt-1 font-semibold">{ratioPct(selected.forward_5_session_drawdown ?? null)}</p></div></div><div className="mt-5 grid gap-6 xl:grid-cols-[.8fr_1.2fr]"><div className="space-y-4"><div><p className="text-label text-muted-foreground">Closest regime factors</p><div className="mt-2 space-y-2">{(selected.matching_factors ?? []).map((factor) => <div key={factor.factor} className="rounded-lg bg-positive/7 p-3"><p className="text-sm font-semibold capitalize">{factor.factor} · {(factor.similarity_score * 100).toFixed(0)}%</p><p className="mt-1 text-xs text-muted-foreground">{factor.explanation}</p></div>)}</div></div><div><p className="text-label text-muted-foreground">Largest differences</p><div className="mt-2 space-y-2">{(selected.divergence_factors ?? []).map((factor) => <div key={factor.factor} className="rounded-lg bg-warning/7 p-3"><p className="text-sm font-semibold capitalize">{factor.factor} · {(factor.similarity_score * 100).toFixed(0)}%</p><p className="mt-1 text-xs text-muted-foreground">{factor.explanation}</p></div>)}</div></div></div><HistoryFeatureComparison current={summary} analogue={selected} /></div></Panel>}
