...
export default function HistoryPage() {
  ...
  return <div className="space-y-5">
    ...
    {result.similar_sessions[0] && <Panel><SectionHeader title="Why the nearest session is similar" description="Deterministic factor-group explanations plus a direct observable-input comparison." /><div className="mt-5 grid gap-6 xl:grid-cols-[.8fr_1.2fr]"><div className="space-y-4"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-muted-foreground">Closest regime factors</p><div className="mt-2 space-y-2">{(result.similar_sessions[0].matching_factors ?? []).map((factor) => <div key={factor.factor} className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-3"><p className="text-sm font-semibold capitalize">{factor.factor} · {(factor.similarity_score * 100).toFixed(0)}%</p><p className="mt-1 text-xs text-muted-foreground">{factor.explanation}</p></div>)}</div></div><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-muted-foreground">Largest differences</p><div className="mt-2 space-y-2">{(result.similar_sessions[0].divergence_factors ?? []).map((factor) => <div key={factor.factor} className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3"><p className="text-sm font-semibold capitalize">{factor.factor} · {(factor.similarity_score * 100).toFixed(0)}%</p><p className="mt-1 text-xs text-muted-foreground">{factor.explanation}</p></div>)}</div></div></div><HistoryFeatureComparison current={summary} analogue={result.similar_sessions[0]} /></div></Panel>}
