...
export default function DiscoverPage() {
  ...
  return <div className="space-y-5">
    ...
    <Panel><form onSubmit={submit}><Label htmlFor="screen-query">Describe your screen</Label><textarea id="screen-query" value={query} onChange={(event) => setQuery(event.currentTarget.value)} rows={3} className="mt-2 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm leading-6 outline-none focus-visible:ring-2 focus-visible:ring-ring" /><div className="mt-3 flex flex-wrap gap-2">{examples.map((example, index) => <button type="button" key={example} onClick={() => setQuery(example)} className="rounded-md border border-border/70 px-2.5 py-1.5 text-left text-[11px] text-muted-foreground hover:border-primary/40 hover:text-foreground">Example {index + 1}</button>)}</div><div className="mt-4 flex items-center gap-3"><Button type="submit" disabled={busy}>{busy ? "Evaluating…" : "Run deterministic screen"}</Button><p className="text-xs text-muted-foreground">Unsupported fields are rejected; no SQL or AI stock selection is generated.</p></div></form>{error && <p role="alert" className="mt-3 text-sm text-negative">{error}</p>}</Panel>
    ...
    <Panel><SectionHeader title="Saved screens" description="Reusable research filters that can later become alert definitions." />{saved.length === 0 ? <div className="mt-4"><DataState kind="empty" title="No saved screens" description="Run and name a screen to preserve its validated filter tree." /></div> : <ul className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{saved.map((item) => <li key={item.id} className="rounded-lg border border-border/60 p-3"><p className="text-sm font-semibold">{item.name}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{item.query_text}</p><div className="mt-3 flex gap-2"><Button size="sm" variant="outline" onClick={() => void run(item)} disabled={busy}><Play className="size-3.5" />Run</Button><Button size="sm" variant="ghost" aria-label={`Delete ${item.name}`} onClick={() => void remove(item)}><Trash2 className="size-3.5" /></Button></div></li>)}</ul>}</Panel>
  </div>;
}
