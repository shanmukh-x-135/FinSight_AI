...
const stockNewsEvidence = { id: 1, headline: "Reliance expands clean-energy investment", publisher: "Reuters", published_at: "2026-08-21T08:00:00Z", source_url: "https://example.com", sentiment_class: "positive" as const, sentiment_score: .48, sentiment_confidence: .82, event_category: "Product Launch", event_confidence: .7, driver: "Product or service announcement", evidence_excerpt: "Reliance announced an expansion of its clean-energy investment.", matched_alias: "RELIANCE INDUSTRIES", entity_match_confidence: .95 };
const freshness = [{ dataset: "market", state: "Fresh", observed_date: "2026-08-21", observed_at: null, expected_trading_date: "2026-08-21", explanation: "Market is current." }, { dataset: "news", state: "Fresh", observed_date: "2026-08-21", observed_at: "2026-08-21T12:00:00Z", expected_trading_date: "2026-08-21", explanation: "News is current." }, { dataset: "universe", state: "Fresh", observed_date: "2026-08-20", observed_at: "2026-08-20T12:00:00Z", expected_trading_date: null, explanation: "Universe is current." }, { dataset: "historical_corpus", state: "Fresh", observed_date: "2026-08-21", observed_at: "2026-08-21T17:00:00Z", expected_trading_date: "2026-08-21", explanation: "History is current." }];
...
const portfolioRisk = { portfolio_id: 1, as_of: "2026-08-21", methodology: "Static current-quantity/current-value weights applied to aligned historical close returns; not actual portfolio performance.", benchmark_symbol: "^NSEI", observations: 252, minimum_observations: 30, data_complete: true, missing_symbols: [], annualized_volatility_percent: 17.4, beta: .91, sharpe_ratio: 1.08, max_drawdown_percent: 8.3, concentration_hhi: .35, momentum_exposure_percent: 3.8, correlation: quotes.flatMap((a) => quotes.map((b) => ({ symbol_x: a.symbol, symbol_y: b.symbol, correlation: a.symbol === b.symbol ? 1 : a.sector === b.sector ? .7 : .35, observations: 252 }))), holding_contributions: quotes.map((q, index) => ({ symbol: q.symbol, weight_percent: [42, 31.35, 26.65][index], return_contribution_percent: [5.2, 2.1, 2.8][index], risk_contribution_percent: [46, 29, 25][index], momentum_20d_percent: [6.8, -1.2, 4.3][index] })), sector_deviation: [{ sector: "Energy", portfolio_weight_percent: 42, benchmark_weight_percent: 12, deviation_percent: 30 }, { sector: "Technology", portfolio_weight_percent: 31.35, benchmark_weight_percent: 15, deviation_percent: 16.35 }, { sector: "Financial Services", portfolio_weight_percent: 26.65, benchmark_weight_percent: 35, deviation_percent: -8.35 }], sector_benchmark_methodology: "NIFTY 100 current constituent market-cap weights.", regime_sensitivity: [{ regime: "broad_positive", sessions: 110, average_daily_return_percent: .12, positive_session_percent: 64 }, { regime: "mixed", sessions: 92, average_daily_return_percent: .03, positive_session_percent: 53 }, { regime: "broad_negative", sessions: 50, average_daily_return_percent: -.18, positive_session_percent: 34 }], stress_scenarios: [{ code: "nifty_down_5", label: "Broad market sell-off", shock: "NIFTY -5%", estimated_impact_percent: -4.55, estimated_value_change: -27218, methodology: "Portfolio beta × -5%; linear single-factor estimate.", is_prediction: false }, { code: "crude_up_10", label: "Crude oil shock", shock: "Crude +10%", estimated_impact_percent: 1.8, estimated_value_change: 10768, methodology: "Published sector shock assumptions.", is_prediction: false }, { code: "financials_down_7", label: "Financial-sector underperformance", shock: "Financials -7%", estimated_impact_percent: -1.87, estimated_value_change: -11187, methodology: "Current Financial-sector weight × -7%.", is_prediction: false }] };
...
async function mockWorkspace(page: Page) {
  await page.route("**/api/v1/**", async (route: Route) => {
    ...
    else if (path === "/api/v1/dashboard/summary") data = { market: { breadth, gainers: [quotes[0], quotes[2]], losers: [quotes[1]] }, ai_market_summary: "Participation is constructive with advancers leading decliners. Momentum remains selective rather than universal.", portfolio, watchlist: quotes.map((q, index) => ({ id: index + 1, ...q, current_price: q.close, pinned: index === 0, sort_order: index })), sectors: [{ sector: "Energy", stock_count: 12, average_change_percent: 1.4 }, { sector: "Technology", stock_count: 14, average_change_percent: -.8 }, { sector: "Financial Services", stock_count: 16, average_change_percent: .6 }], technical: { as_of: "2026-08-21", stocks_with_indicators: 50, average_rsi: 56.4, bullish_rsi_count: 30, overbought_count: 3, oversold_count: 2, above_ema20_count: 31, above_ema50_count: 28, positive_macd_count: 29, average_atr_percent: 1.6 }, sentiment: quotes.map((q, index) => ({ symbol: q.symbol, latest_sentiment: [.42, -.18, .12][index], availability: "available", confidence: .75, article_count: 1 })), opportunities: [rec], risk_alerts: [], history, generation: usage, freshness };
    else if (path === "/api/v1/market/workspace") data = { universe: "NIFTY100", stocks: quotes, breadth, sectors: [{ sector: "Energy", stock_count: 12, average_change_percent: 1.4 }, { sector: "Technology", stock_count: 14, average_change_percent: -.8 }, { sector: "Financial Services", stock_count: 16, average_change_percent: .6 }], technical: { as_of: "2026-08-21", stocks_with_indicators: 50, average_rsi: 56.4, bullish_rsi_count: 30, overbought_count: 3, oversold_count: 2, above_ema20_count: 31, above_ema50_count: 28, positive_macd_count: 29, average_atr_percent: 1.6 }, heatmap: quotes.map((quote, index) => ({ symbol: quote.symbol, name: quote.name, sector: quote.sector, as_of: quote.date, change_percent: quote.change_percent, market_cap: [19000000000000, 12000000000000, 14500000000000][index], sentiment: [.42, -.18, .12][index], sentiment_availability: "available", rsi_14: quote.rsi_14, signal: quote.trend })), sector_rotation: [{ sector: "Energy", stock_count: 12, return_1d: 1.4, return_5d: 3.2, return_20d: 6.8, momentum_regime: "leader" }, { sector: "Financial Services", stock_count: 16, return_1d: .6, return_5d: 2.1, return_20d: 4.4, momentum_regime: "improving" }, { sector: "Technology", stock_count: 14, return_1d: -.8, return_5d: -1.5, return_20d: 2.2, momentum_regime: "weakening" }], economic_events: { provider: "Trading Economics", status: "ok", events: [{ event_id: "rbi", date: "2026-08-25T10:00:00Z", country: "India", category: "Interest Rate", name: "RBI policy minutes", importance: 3, reference: null, source: "RBI", source_url: null, actual: null, forecast: null, previous: null }] }, sentiment: quotes.map((q, index) => ({ symbol: q.symbol, latest_sentiment: [.42, -.18, .12][index], availability: "available", confidence: .75, article_count: 1 })) };
    else if (path === "/api/v1/market/stocks") data = quotes;
    else if (path === "/api/v1/market/universes") data = [{ code: "NIFTY50", label: "NIFTY 50", expected_constituents: 50, active_constituents: 50, initialized: true, preferred: false, source_url: "https://www.niftyindices.com/IndexConstituent/ind_nifty50list.csv" }, { code: "NIFTYNEXT50", label: "NIFTY Next 50", expected_constituents: 50, active_constituents: 50, initialized: true, preferred: false, source_url: "https://www.niftyindices.com/IndexConstituent/ind_niftynext50list.csv" }, { code: "NIFTY100", label: "NIFTY 100", expected_constituents: 100, active_constituents: 100, initialized: true, preferred: true, source_url: "https://www.niftyindices.com/IndexConstituent/ind_nifty100list.csv" }];
    else if (path === "/api/v1/market/heatmap") data = quotes.map((quote, index) => ({ symbol: quote.symbol, name: quote.name, sector: quote.sector, as_of: quote.date, change_percent: quote.change_percent, market_cap: [19000000000000, 12000000000000, 14500000000000][index], sentiment: [.42, -.18, .12][index], sentiment_availability: "available", rsi_14: quote.rsi_14, signal: quote.trend }));
    else if (path === "/api/v1/market/sector-rotation") data = [{ sector: "Energy", stock_count: 12, return_1d: 1.4, return_5d: 3.2, return_20d: 6.8, momentum_regime: "leader" }, { sector: "Financial Services", stock_count: 16, return_1d: .6, return_5d: 2.1, return_20d: 4.4, momentum_regime: "improving" }, { sector: "Technology", stock_count: 14, return_1d: -.8, return_5d: -1.5, return_20d: 2.2, momentum_regime: "weakening" }];
    ...
    else if (path === "/api/v1/news/sentiment") data = quotes.map((q, index) => ({ symbol: q.symbol, latest_sentiment: [.42, -.18, .12][index], availability: "available", confidence: .75, article_count: 1 }));
    ...
    else if (/\/market\/stocks\/[^/]+\/attribution$/.test(path)) data = { symbol: "RELIANCE.NS", as_of: "2026-08-21", change_percent: 1.88, certainty: "likely_contributors_not_proven_causes", summary: "Observable contributors evaluated; associations are not proven causes.", drivers: [{ rank: 1, category: "company_news", label: "Company-specific news", observation: "1 verified article association; aggregate sentiment +0.48", direction: "bullish", relevance: "high", confidence: .78, value_percent: .48, evidence_article_ids: [1] }, { rank: 2, category: "sector", label: "Energy return", observation: "Sector average return +1.40%", direction: "bullish", relevance: "high", confidence: .8, value_percent: 1.4, evidence_article_ids: [] }], evidence_conflict: { consensus: "bullish", confidence: .74, conflict_detected: false, signals: [{ source: "Technical", direction: "bullish", confidence: .72, observation: "Price is above EMA20; MACD histogram is positive" }, { source: "News", direction: "bullish", confidence: .78, observation: "Verified positive article evidence" }] } };
    ...
    else if (/\/news\/sentiment\//.test(path)) data = { symbol: "RELIANCE.NS", name: "Reliance Industries", latest_sentiment: .42, availability: "available", confidence: .78, article_count: 1, positive_count: 1, negative_count: 0, neutral_count: 0, evidence: [stockNewsEvidence], series: Array.from({ length: 14 }, (_, index) => ({ date: `2026-08-${String(index + 1).padStart(2, "0")}`, avg_sentiment: Math.sin(index / 3) * .5, article_count: 3 + index % 4, positive_count: 2, negative_count: 1, neutral_count: 1 })) };
    else if (path === "/api/v1/news") data = [{ id: 1, source: "Reuters", url: "https://example.com", title: stockNewsEvidence.headline, summary: "", published_at: stockNewsEvidence.published_at, sentiment_label: "positive", sentiment_score: .48, sentiment_confidence: .82, event_category: "Product Launch", event_confidence: .7, driver: stockNewsEvidence.driver, evidence_excerpt: stockNewsEvidence.evidence_excerpt, tags: ["RELIANCE.NS"], associations: [{ symbol: "RELIANCE.NS", matched_alias: "RELIANCE INDUSTRIES", entity_match_confidence: .95 }] }];
    ...
    else if (path === "/api/v1/portfolios/1/risk") data = portfolioRisk;
    ...
    else if (path === "/api/v1/strategies") data = [];
    else if (path === "/api/v1/discovery/screens") data = [];
    else if (path === "/api/v1/admin/jobs/eod/status") data = { pipeline_name: "eod_market_intelligence", requested_trading_date: null, rerun_recommended: false, health: "healthy", operator_explanation: "Every required EOD step completed and the session is safe to no-op on rerun.", freshness, run: { id: 19, pipeline_name: "eod_market_intelligence", target_trading_date: "2026-08-21", correlation_id: "visual-eod-run", status: "completed", attempt_count: 1, counters: {}, last_error_summary: null, started_at: "2026-08-21T11:00:00Z", heartbeat_at: "2026-08-21T11:10:00Z", completed_at: "2026-08-21T11:10:00Z", created_at: "2026-08-21T11:00:00Z", updated_at: "2026-08-21T11:10:00Z", steps: [{ step_name: "market", sequence: 0, status: "completed", attempt_count: 1, counters: { requested: 100, succeeded: 100 }, last_error_summary: null, started_at: "2026-08-21T11:00:00Z", heartbeat_at: null, completed_at: "2026-08-21T11:05:00Z" }, { step_name: "news", sequence: 1, status: "completed", attempt_count: 1, counters: { fetched: 128, sentiment_days_updated: 75 }, last_error_summary: null, started_at: "2026-08-21T11:05:00Z", heartbeat_at: null, completed_at: "2026-08-21T11:07:00Z" }, { step_name: "history", sequence: 2, status: "completed", attempt_count: 1, counters: { sessions_indexed: 199, dimension: 25 }, last_error_summary: null, started_at: "2026-08-21T11:07:00Z", heartbeat_at: null, completed_at: "2026-08-21T11:10:00Z" }] } };
...
test("major research screens remain responsive and visually inspectable", async ({ page }, testInfo) => {
  ...
  const routes: { route: string; heading: string | RegExp }[] = [
    ...
    { route: "strategies", heading: "Strategy Lab" },
    { route: "discover", heading: "Discover" },
    { route: "operations", heading: "EOD Operations" },
