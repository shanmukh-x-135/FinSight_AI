...
| 6 — AI Intelligence (RAG) | `v0.7.0-ai-intelligence` | ✅ done | RAG: context builder (relevant slices only) → deterministic recommendation engine (score/rank/evidence/confidence/risks — **LLM never ranks**) → prompt builder (versioned, constrained) → LLM prose (DeterministicNarrator default; **Gemini integrated+selectable**, optional in requirements-ai.txt) → explainability validation → structured JSONB report (migration 0007: reports + chat_history). Endpoints: reports generate/list/get, recommendations. ~97% coverage. Live: real evidence-backed report, **deterministic ranking + reproducible reports verified**, no price prediction. See `docs/ai-intelligence.md`, `docs/prompts/`. |
| 7 — Dashboard UI | `v0.8.0-dashboard` | ✅ done | Demoable app. Shared component library **built first** (`MetricCard`, `StockCard`, `AIInsightCard`, `EvidencePanel`, `Heatmap`, `DataTable`) then reused across screens: Dashboard (batched `GET /dashboard/summary` — new `app/dashboard/` module), Market Intelligence (breadth + sector heatmap + gainers/losers + AI analysis), Historical Similarity (stats + outcomes + similar sessions), updated Portfolio (AI insights on holdings). **Research Mode "Show Evidence"** = one reusable `EvidencePanel` on every AI insight, wired to real indicators/analogs/risks/confidence — no mocks. New backend read: `GET /market/sectors` (heatmap). Responsive (desktop nav → mobile scroll strip). Vitest+RTL component tests (10, incl. Show Evidence) + backend tests (182 pass). Live-verified end-to-end on real NSE data (RELIANCE watch conf 54%, evidence-backed). See `docs/frontend-components.md`. |
| 8 — Reports | `v0.9.0-reports` | ✅ done | New `app/reports/` module (browse→open→export; generation stays in intelligence, reports table reused — no migration): `GET /reports` **filterable** (type/date range) **+ paginated** (limit/offset), `GET /reports/{id}` (ownership 404), `GET /reports/{id}/export?format=markdown\|pdf` (downloadable file). **Markdown = single source of truth; PDF rendered from it** via **fpdf2** (pure-Python, no system libs). PDF edge cases handled: Unicode sanitized (em-dash/₹/curly quotes/CJK → safe latin-1, never crashes), long text wraps, missing sections skipped. Report endpoints moved out of `intelligence/routes.py` (Phase 6 tests still pass). Frontend: `/reports` (filters + generate + pagination) + `/reports/[id]` (**Report page template** — Exec Summary→Analysis→Recommendations w/ Show Evidence→Appendix) + Markdown/PDF export via authed blob download. ~197 backend tests pass (16 new reports tests incl. long-text/special-char/missing-section export). Live-verified: real report generated, filtered, exported to valid Markdown + PDF. See `docs/reports.md`. |
